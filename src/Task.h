...
public:
   ...
   */
  void did_waitpid(WaitStatus status, bool recurse = false);
  ...
  bool is_in_rr_page_syscall() {
    auto p = ip().to_data_ptr<void>();
    return AddressSpace::rr_page_start() <= p &&
      p < AddressSpace::rr_page_syscall_end();
  }

  bool is_in_rr_page_thunk() {
    return is_in_rr_page() && !is_in_rr_page_syscall();
  }
